...
void AUDIO_PlaybackHandler::init(int _sample_rate, int _channels, int _skip_bytes, int _audio_format, int _bit_rate, int _skip_end_bytes) {
	if( this->is_init ) {
	...
	this->print_debug_info("init() parameter - bit rate         : [%d]\n", this->bit_rate);
...
void AUDIO_PlaybackHandler::run(string _file_path) {
	int	 frame_latency = 0;
	int	 read_count = 0;
	int  num_period_size = NUM_PERIOD_SIZE;
	char *feed_data = NULL;
	int  feed_size  = sizeof(HEADER_INFO_t) + num_period_size;
	
	char arr_play_data[feed_size], arr_pause_data[feed_size];
	char arr_read_data[num_period_size];
	char arr_pcm_data[num_period_size];
	...
	int  	decode_count	= 0;
	int  	decode_offset	= 0;
	bool	is_mp3_encode	= false;
	
	int		num_time_correct = 0;
	...
	// status : pause
	t_header_info.status 		= 0x02;
	t_header_info.encode_type	= 0x00;
	t_header_info.mp3_quality 	= 0;
	t_header_info.data_size		= num_period_size;
	t_header_info.sample_rate	= this->sample_rate;
	t_header_info.channels		= this->channels;
	memset(arr_pause_data, 0x00, sizeof(arr_pause_data));
	memcpy(arr_pause_data, &t_header_info, sizeof(HEADER_INFO_t));
	
	// status : common
	t_header_info.status	  	= 0x01;
	t_header_info.data_size		= num_period_size;
	t_header_info.sample_rate	= this->sample_rate;
	t_header_info.channels		= this->channels;
	...
	if( is_mp3_encode ) {
		t_header_info.encode_type 	= 0x01;
		t_header_info.mp3_quality 	= this->bit_rate / 1000;
	...
	for( int idx = 0 ; idx < (int)sizeof(LIST_WAV_TIME_CORRECT) ; idx++ ) {
		if( LIST_WAV_TIME_CORRECT[idx][0] == this->sample_rate ) {
			num_time_correct = LIST_WAV_TIME_CORRECT[idx][1];
			
			if( this->channels == 2 ) {
				num_time_correct = num_time_correct * 1.5;
			}
			break;
		}
	}
	
	memcpy(arr_play_data, &t_header_info, sizeof(HEADER_INFO_t));
	
	frame_latency = (num_period_size * 1000 * 1000) / (this->sample_rate * this->channels * 2);
	
	// wav/pcm ID3/TAG header skip
	for_each(ext_type.begin(), ext_type.end(), [](char &_char) {
		_char = tolower(_char);
	});
	...
	if( fp == NULL ) {
		this->print_debug_info("run() fopen failed : [%02d] %s\n", errno, strerror(errno));
	}
	...
	this->print_debug_info("run() start read file : [%s]\n", _file_path.c_str());
	this->print_debug_info("run() time correction : [%d]\n", num_time_correct);
	
	this->is_run = true;
	
	struct timeval t_time_loop_begin, t_time_loop_end;
	struct timeval t_time_read_begin, t_time_read_end;
	struct timeval t_time_decode_end;

	int num_read_time = 0;
	int num_loop_time = 0;
	int num_decoded_time = 0;
	int calc_frame_latency = 0;
	
	gettimeofday(&t_time_loop_begin, NULL);

	bool is_skip_end_bytes = false;
	int  sum_end_skip_size = 0;

	while( !this->is_loop && !feof(fp) ) {
		gettimeofday(&t_time_read_begin, NULL);
		
		if( this->is_pause ) {
			if( this->playback_handle != NULL ) {
				this->playback_handle(arr_pause_data, feed_size);
			}
			usleep(frame_latency);
			
		} else {
			if( this->skip_end_bytes != 0 && (sum_end_skip_size + num_period_size) > this->skip_end_bytes ) {
				num_period_size = this->skip_end_bytes - sum_end_skip_size;
			...
			if( is_mp3_encode ) {
				t_decode_info = mp3_decoder.decode(arr_read_data, read_count);
				...
				gettimeofday(&t_time_decode_end, NULL);
				num_decoded_time = ((int)this->calc_diff_time(t_time_read_begin, t_time_decode_end));
				
				decode_offset = 0;
				for( int idx = 0 ; idx < loop_count ; idx++ ) {
					data_size = arr_decode_length[idx];
					decode_count = data_size / num_period_size;
					
					for( int p_idx = 0 ; p_idx < decode_count ; p_idx++ ) {
						gettimeofday(&t_time_read_begin, NULL);
						
						memcpy(arr_pcm_data, feed_data + decode_offset, num_period_size);
						
						decode_offset += num_period_size;

						if( this->playback_handle != NULL ) {
							memcpy(arr_play_data + sizeof(HEADER_INFO_t), arr_pcm_data, num_period_size);
							this->playback_handle(arr_play_data, feed_size);
						}
						
						gettimeofday(&t_time_read_end, NULL);
						num_read_time = ((int)this->calc_diff_time(t_time_read_begin, t_time_read_end));
						
						calc_frame_latency = frame_latency - (num_decoded_time / decode_count / loop_count) - num_read_time - this->num_send_delay + num_time_correct - 2;
						if( calc_frame_latency > 0 ) { 
							usleep(calc_frame_latency);
						}
					}
				}
				...
				if( this->playback_handle != NULL ) {
					memcpy(arr_play_data + sizeof(HEADER_INFO_t), arr_read_data, num_period_size);
					this->playback_handle(arr_play_data, feed_size);
				}
				gettimeofday(&t_time_read_end, NULL);
				num_read_time = ((int)this->calc_diff_time(t_time_read_begin, t_time_read_end));
				
				calc_frame_latency = frame_latency - num_read_time - this->num_send_delay + num_time_correct;
				if( calc_frame_latency > 0 ) { 
					usleep(calc_frame_latency);
				}
			}
	...
	gettimeofday(&t_time_loop_end, NULL);
	num_loop_time = ((int)this->calc_diff_time(t_time_loop_begin, t_time_loop_end));
	this->print_debug_info("run() loop time : [%d]\n", num_loop_time);
	...
	memset(arr_play_data, 0x00, sizeof(arr_play_data));
	memcpy(arr_play_data, &t_header_info, sizeof(HEADER_INFO_t));
...
double AUDIO_PlaybackHandler::calc_diff_time(struct timeval _x, struct timeval _y) {
...
int AUDIO_PlaybackHandler::get_header_size(void) {
	
	return sizeof(HEADER_INFO_t);
}

int AUDIO_PlaybackHandler::get_period_size(void) {
	
	return NUM_PERIOD_SIZE;
}

void AUDIO_PlaybackHandler::set_period_delay(int _delay) {
	this-> num_send_delay = _delay;
