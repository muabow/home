...
int MAIN_Handler::query(string _table_name, string _query) {
	this->mutex_db_handler.lock();
	...
	this->print_debug_info("query() [%s]\n", _query.c_str());
	int result = sql_handler.query(_query);
...
bool MAIN_Handler::is_exist_db_colume(string _table, string _colume) {
	this->mutex_db_handler.lock();
...
int MAIN_Handler::add_db_colume(string _table, string _colume, string _type) {
	this->mutex_db_handler.lock();
