...
void SOCKET_UnicastClient::set_connect_handler(void (*_func)(string, bool, int)) {
...
void SOCKET_UnicastClient::execute(void) {
	...
	int		data_size = SIZE_RECV_DATA + sizeof(HEADER_INFO_t);
	...
	while( !this->is_loop ) {
		while( !this->is_loop ) {	// alive check loop
			...
			if( (FD_ISSET(this->socket_fd, &t_write_fds) > 0) && (opt_value_error == 0) ) { 
				...
				if( this->event_connect_handle != NULL ) {
					this->event_connect_handle("unicast", this->is_run, NETWORK_CONNECT_FAILED);
				}
		...
		if( !this->is_loop ) {
			...
			if( this->event_connect_handle != NULL ) {
				this->event_connect_handle("unicast", this->is_run, NETWORK_CONNECT_NORNAL);
			}
		}
		...
		while( !this->is_loop ) { // recv data loop
			...
			memset(recv_data, 0x00, data_size);
			if( (rc = recv(this->socket_fd, recv_data, data_size, 0)) < 0 ) {
				this->print_debug_info("execute() recv() failed : [%02d] %s\n", errno, strerror(errno));
				break;
			
			} else if( rc == 0 ) {
				this->print_debug_info("execute() recv() [data] Disconnected by peer.. \n");
				break;
			}
...
void SOCKET_MulticastClient::set_connect_handler(void (*_func)(string, bool, int)) {
	this->print_debug_info("set_connect_handler() set event function\n");
	this->event_connect_handle = _func;
	...
	while( !this->is_loop ) {
		...
		if( !this->is_loop ) {
			if( !is_connected ) {
				...
				if( this->event_connect_handle != NULL ) {
					this->event_connect_handle("multicast", this->is_run, NETWORK_CONNECT_FAILED);
				}
			...
			if( this->event_connect_handle != NULL ) {
				this->event_connect_handle("multicast", this->is_run, NETWORK_CONNECT_NORNAL);
			}
		}
