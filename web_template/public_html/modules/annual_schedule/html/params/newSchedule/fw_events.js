...
$(document).ready(function() {
	...
	var year_v = '/modules/annual_schedule/html/params/newSchedule/bootstrap/index.php';
...
*/
	var addEvent = function(start, end) {
		...
		if (fileType == "mp3"){
			...
			Contents = $('#edited_script').val();
		}
		else{
			alert("Action-type is not selected!");
			return;
		}
		...
		if(title)
		{
			...
			if(noError)
			{
				...
				$.ajax({
					type:"POST",
					url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=add",
					data: dataString,
					dataType: 'json',
		...
		if(title != null && title != ''){
			...
			if(noError) {
				...
				var dataString = '&event_id='+ event.id + '&title=' + title + '&datelist=' + datelist;
		           	$.ajax({
			        	type:"POST",
			            url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=scheduleCopy",
			            data: dataString,
			            dataType: 'json',
	...
	var updateEvent = function(event) {
		...
		if (action_type == "mp3"){
			...
			action_contents = $('#edited_script').val();
		}
		else{
			alert("Action Type is not selected!");
			return;
		...
		if(title != null && title != ''){
			...
			if(noError) {
				...
				dataString += '&dow_0=' + $('#dow_0').is(':checked') + '&dow_1=' + $('#dow_1').is(':checked') + '&dow_2=' + $('#dow_2').is(':checked') + '&dow_3=' + $('#dow_3').is(':checked') + '&dow_4=' + $('#dow_4').is(':checked') + '&dow_5=' + $('#dow_5').is(':checked') + '&dow_6=' + $('#dow_6').is(':checked');
		           	$.ajax({
			        	type:"POST",
			            url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=update",
			            data: dataString,
			            dataType: 'json',
						success:function(result){
							//event = applyToObject(event, result.event);
		...
		$("#dialog-message").dialog({
			...
			[{
		        		...
		        		click: function (a,b)
		        		{
							...
							{
								$('#error_message').html('');
								var typeAction = $("#edited_progress").val();
								if (typeAction == "Script" || typeAction == "TTS"){
...
//				$("#file_area option").removeAttr("selected");
				...
				$("#script_area").show();
				$("#edited_script").val(event.contents);
				$("#file_area").hide();
				$("#tts_area").hide();
				$("#no_action").hide();
			}
...
//				title: PopupTitle + ' ' + event.name,
	...
	var onEventDropEvent = function(event) {
		...
		move_to_start_time = moment(event.start).format('HH:mm');
		move_from = moment(event.start._i).format('YYYY-MM-DD'); // 이전 날짜
		move_to_end_time = moment(event.end).format('HH:mm');

		diff_date = dateDiff(move_from, move_to);
		if ((event.repeat_everyday == "1" || (event.day_of_week != undefined && event.day_of_week != "")) /*&& diff_date != 0*/)
		{
			sd = new Date(event.start_base.substr(0,10)+" "+move_to_start_time);
			ed = new Date(event.end_base.substr(0,10)+" "+move_to_end_time);
		...
		$.ajax({
			type:"POST",
			url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=resize",
			data: dataString,
			dataType: 'json',
...
*/
			...
			layer ='<div id="events-layer" class="fc-transparent" style="position:absolute; top:3px; right:2px; z-index:100">'
					+ '<span title="delete" id="delbut'+fixId+'" style="display:inline;"><img src="/modules/annual_schedule/html/params/newSchedule/img/icon_del.png"></span>'
					+ '</div>';
	...
	$( "#delete_schedule_button" ).click(function() {
		...
		if($('#this_schedule').is(':checked') && (selectedRepeat != "" || selectedEveryday == "1" || isDOWCheck())) {
			...
			$.ajax({
				type:"POST",
				url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=del_this",
				data: dataString,
				success:function(html){
					$('#calendar').fullCalendar('refetchEvents');
					var iframe = document.getElementById('yearIframe');
			...
			$.ajax({
				type:"POST",
				url: "/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=del",
				data: dataString,
				success:function(html){
					$('#calendar').fullCalendar('refetchEvents');
					var iframe = document.getElementById('yearIframe');
	...
	$('#calendar').fullCalendar({
		customButtons: {
			Holiday: {
				text: str_holiday,
				click: function() {
					window.open('/modules/annual_schedule/html/params/newSchedule/holiday_list.php');
				}
			},
			Download: {
				text: str_download,
				click: function() {
					location.href=("/modules/annual_schedule/html/params/newSchedule/file_download.php");
				}
			},
			Upload: {
				text: str_upload,
		...
		events:
		{
			url: '/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=get',
		    type: 'POST',
		    data:
		    {
				m: m
		...
		[
			{
				url: '/modules/annual_schedule/html/params/newSchedule/fw_events.php?action=getHolidays&m='+m,
				color: 'lightgray',
				textColor:'red'
			},
