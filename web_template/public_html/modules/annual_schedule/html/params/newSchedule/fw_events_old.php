...
$TEMP_SCHEDULE_FILE = "/opt/interm/www/html/auth_admin/params/newSchedule/info_schedule_temp.xml";
$SCHEDULE_FILE = "/opt/interm/www/html/auth_admin/params/newSchedule/info_schedule.xml";
...
include_once("/opt/interm/www/html/auth_admin/params/newSchedule/fw_config.php");

function json_encode2($content) {

	require_once '/opt/interm/www/html/auth_admin/params/newSchedule/libs/JSON.php';
...
if (file_exists($file_name)) {
	...
	foreach($xml->children() as $value) {
		...
		if (isset($value->schedule->dow))
			$dow = $value->schedule->dow->__toString();
		else
			$dow = "";
		...
		if (($repeat_everyday == "0" || $repeat_everyday == "") && $dow == "") {
			...
			$found = false;
			foreach ($list as $cal_list) {
				if ($cal_list['start'] == $begin_result && $cal_list['end'] == $end_result && $cal_list['id'] == $created_at)
					$found = true;
			}
			if ($found == true)
				continue;
			...
			/*if ($dow != "") {
				$list[$i]['dow'] = "[". $dow . "]";
				$range = array();
				$range["start"] = "2019-04-03";
				$range["end"] = "2019-04-21";
				$list[$i]['ranges'] = $range;
			}*/
			$i++;
		}
	...
	foreach($xml as $value) {
		...
		switch($repeat)
		{
			...
			for($t = $start_Y; $t <= $end_Y; $t++)
			{
				...
				if ($cal_date == ($begin_date."T".$begin_h.":".$begin_m) && $repeat_everyday == "0" && $dow == "")
					continue;
				...
				if ($dow == "") {
					if ($repeat_everyday == "1") {
						...
						for (;$begin_datetime <= $end_datetime; $begin_datetime->add(new DateInterval('P1D')), $next_day_datetime->add(new DateInterval('P1D'))) {
							...
							$found = false;
							foreach ($list as $cal_list) {
								if ($cal_list['start'] == $begin_result_daily && $cal_list['end'] == $end_result_daily && $cal_list['id'] == $created_at)
									$found = true;
							}
							if ($found == true)
								continue;
						...
						$found = false;
						foreach ($list as $cal_list) {
							if ($cal_list['start'] == $t.substr($begin_result, 4) && $cal_list['end'] == $t.substr($end_result, 4) && $cal_list['id'] == $created_at)
								$found = true;
						}
						if ($found == true)
							continue;
					...
					for (;$begin_datetime <= $end_datetime; $begin_datetime->add(new DateInterval('P1D')), $next_day_datetime->add(new DateInterval('P1D'))) {
						...
						$same_dow = false;
						foreach ($dow_list as $dow_day) {
							if ($dow_day == $begin_datetime->format("w")) {
								$same_dow = true;
								break;
							}					
						}
			...
			for($t = $two_month_cal_adj; $t < $max_month; $t++)
			{
				$cal_date = date("Y-m", mktime(0,0,0, $start_M + $t, 1, $start_Y)).substr($begin_result, 7);
				
				if (substr($begin_result, 0, 7) == substr($end_result, 0, 7))
					$cal_date_end = date("Y-m", mktime(0,0,0, $start_M + $t, 1, $start_Y)).substr($end_result, 7);
				else
					$cal_date_end = date("Y-m", mktime(0,0,0, $start_M + 1 + $t, 1, $start_Y)).substr($end_result, 7);
				...
				if ($dow == "") {
					...
					if ($repeat_everyday == "1") {
						...
						$next_day_datetime = new DateTime($cal_date);
						$next_day_datetime->add(new DateInterval('P1D'));
						$end_datetime = new DateTime($cal_date_end);//new DateTime(date("Y-m", mktime(0,0,0, $start_M + $t, 1, $start_Y)).substr($end_daily, 7));
						//echo substr($begin_result, 0, 7)."====".substr($end_result, 0, 7);
						//echo "....".$begin_datetime->format("Y-m-d")."...".$end_datetime->format("Y-m-d");
						for (;$begin_datetime <= $end_datetime; $begin_datetime->add(new DateInterval('P1D')), $next_day_datetime->add(new DateInterval('P1D'))) {
							$today = $begin_datetime->format("Y-m-d");
						...
						$found = false;
						foreach ($list as $cal_list) {
							if ($cal_list['start'] == date("Y-m", mktime(0,0,0, $start_M + $t, 1, $start_Y)).substr($begin_result, 7) && $cal_list['end'] == date("Y-m", mktime(0,0,0, $start_M + $t, 1, $start_Y)).substr($end_result, 7) && $cal_list['id'] == $created_at)
								$found = true;
						}
						if ($found == true)
							continue;
					...
					$next_day_datetime = new DateTime($cal_date);
					$next_day_datetime->add(new DateInterval('P1D'));
					$end_datetime = new DateTime($cal_date_end);//new DateTime(date("Y-m", mktime(0,0,0, $end_m + $t, 1, $start_Y)).substr($end_daily, 7));
					//echo "....".$begin_datetime->format("Y-m-d")."...".$end_datetime->format("Y-m-d");
					for (;$begin_datetime <= $end_datetime; $begin_datetime->add(new DateInterval('P1D')), $next_day_datetime->add(new DateInterval('P1D'))) {
						$today = $begin_datetime->format("Y-m-d");
				...
				if ($dow == "") {
					if ($repeat_everyday == "1") {
						...
						for (;$begin_datetime <= $end_datetime; $begin_datetime->add(new DateInterval('P1D')), $next_day_datetime->add(new DateInterval('P1D'))) {
							...
							$found = false;
							foreach ($list as $cal_list) {
								if ($cal_list['start'] == $cal_date && $cal_list['end'] == date("Y-m-d", mktime(0,0,0, $start_M, $start_D + $chk_week1 + $k*7, $start_Y)).substr($end_result, 10) && $cal_list['id'] == $created_at)
									$found = true;
							}
							if ($found == true)
								continue;
...
function scheduleCopy()
{
	...
	$title = $_POST['title'];
	$datelist = json_decode2(html_entity_decode($_POST['datelist']));
		...
		if ($schedule_created_at == $created_at) {
			...
			$_id = (int)substr($created_at,3,3);
			 foreach ($datelist as $date) {
				...
				if (isset($schedule->schedule->dow))
					$result_schedule->addChild('dow',$schedule->schedule->contents->__toString());
				$format = "id_%03d";
...
//	$auto_yn = (isset($_POST['auto_playback'])) ? '1' : '0';
	...
	$dow = "";
	if ($_POST['dow_0'] == "true") $dow = "0";
	if ($_POST['dow_1'] == "true") $dow = ($dow == "") ? "1" : $dow . ",1";
	if ($_POST['dow_2'] == "true") $dow = ($dow == "") ? "2" : $dow . ",2";
	if ($_POST['dow_3'] == "true") $dow = ($dow == "") ? "3" : $dow . ",3";
	if ($_POST['dow_4'] == "true") $dow = ($dow == "") ? "4" : $dow . ",4";
	if ($_POST['dow_5'] == "true") $dow = ($dow == "") ? "5" : $dow . ",5";
	if ($_POST['dow_6'] == "true") $dow = ($dow == "") ? "6" : $dow . ",6";
...
function getDuplicated($event_id)
{
	...
	$new_event_list = json_decode2(getEvent_list($start_date->format('Y-m-d'), $end_date->format('Y-m-d'), $TEMP_SCHEDULE_FILE));
	$event_list = json_decode2(getEvent_list($start_date->format('Y-m-d'), $end_date->format('Y-m-d'), $GLOBALS['SCHEDULE_FILE']));
...
//	$created_at = date("Y-m-d H:i:s");
	...
	$result_schedule->addChild('dow',$dow);
...
function delAnEvent() 
{
	...
	foreach($xml1 as $schedule) {
		...
		if ($schedule_created_at == $created_at) {
			$prev_begin_date = new DateTime($schedule->schedule->begin_date->__toString());
			$prev_end_date = new DateTime($schedule->schedule->end_date->__toString());
			//$dow = new DateTime($schedule->schedule->dow->__toString());
	...
	foreach($xml1 as $schedule) {
		...
		if ($schedule_created_at == $created_at) {
			$dow = "";
			if (isset($schedule->schedule->dow))
				$dow = $schedule->schedule->dow->__toString();
			$dow_list = explode(',',$dow);
			$dow_cnt = 0;
			if ($dow != "" && $schedule->schedule->repeat->__toString() == "") {
				for ($cnt=0;$date_diff>$cnt;$cnt++) {
					foreach ($dow_list as $today_dow) {
						if ($prev_begin_date->add(new DateInterval('P1D'))->format("w") == $today_dow)
							$dow_cnt++;
					}
				}
				if ($dow_cnt == $schedule->schedule->excludes->children()->start_date->count()) {
					unset($xml1->note[$i]);
				}
			} else {
...
function getHolidays() 
{
	...
	$xml4 = simplexml_load_file('/opt/interm/www/html/auth_admin/params/newSchedule/info_holiday.xml');
	$i = 0;
	$list = array();
...
function setDateFormat() {
	...
	file_put_contents('/opt/interm/www/html/auth_admin/params/newSchedule/date_format.xml',$doc->saveXML());
}
?>
