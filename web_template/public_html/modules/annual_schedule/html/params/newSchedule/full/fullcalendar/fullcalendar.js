...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if(event.progress == 100) progress = " style='text-decoration: line-through; -moz-opacity: 0.7;opacity:0.7;filter: alpha(opacity=70);' "; else progress = ""; 
...
*/
/* modified start */
			// A case, the duration of schedule is smaller than 20 minutes.
			// Calculating collision parameter is schedule duration,
			// but collision is occurred on display object of schedule,
			// that height is bigger then real duration of schedule.
			// So, change the bottom value of schedule to display object size.
			diff_min = seg.end.diff(seg.start, "minutes");
			if (diff_min < 20)
				seg.bottom = this.computeDateTop(seg.end.clone().add(20-diff_min, "minutes"), seg.start);
			else
				seg.bottom = this.computeDateTop(seg.end, seg.start);
/* modified end */
		}
