...
function getEvent_list($start_t, $end_t, $file_name)
{
	...
	$start_date_adj = new DateTime($start_t);
	$start_date_adj->sub(new DateInterval('P' . (string)date("w", strtotime($start)) . 'D'));
	$start = $start_date_adj->format("Y-m-d");
...
//	$auto_yn = (isset($_POST['auto_playback'])) ? '1' : '0';
	...
	$begin[0] = (strlen($begin[0]) < 2) ? "0" . $begin[0] : $begin[0];
	$begin[1] = (strlen($begin[1]) < 2) ? "0" . $begin[1] : $begin[1];
	$end = explode(':',$end_time);
	$end[0] = (strlen($end[0]) < 2) ? "0" . $end[0] : $end[0];
...
function getDuring()
{
	...
	if ($cnt > 1){	// 2 over file name
		...
		for($i = 0; $i < $cnt; $i++){
			...
			$re_file_name = '/opt/interm/public_html/modules/annual_schedule/html/data/files/'.$dur_name;
