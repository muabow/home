...
#define FACTORY_RESET		"factory_reset"				// DeviceFactory 에 정의된 Key
#define LIVE_LED			"live_led"					// DeviceFactory 에 정의된 Key
...
//	printf("\n[Factory Reset] Start ========================================\n");
	...
	try {
		_liveLED = new BlinkLED(LIVE_LED, 1000, false);		// DeviceFactory
		...
		GPIO* factoryReset = DeviceFactory::createGPIO(FACTORY_RESET);

		while (!quit.load())
		{
			const bool currButtonPressed = factoryReset->read() == GPIO::LOW ? true : false;
