...
int AUDIO_Handler::update_database_status(string _type, string _field, string _value, string _format) {
	...
	while( true ) {
		...
		this->print_debug_info("update_database_status() update set [%s] value [%s]\n", _field.c_str(), _value.c_str());
		...
		if( str_status.compare(_value) == 0 ) {
			...
			this->print_debug_info("update_database_status() update error, retry update : set : [%s], current : [%s]\n",str_status.c_str(), _value.c_str());
		}
	}
...
int AUDIO_Handler::update_database_status(string _type, string _field, string _value) {
	this->mutex_db_handler.lock();
...
int AUDIO_Handler::update_database_status(string _type, string _field, int _value) {
	this->mutex_db_handler.lock();
	...
	while( true ) {
		...
		this->print_debug_info("update_database_status() update set [%s] value [%d]\n", _field.c_str(), _value);
		...
		if( stoi(str_status) == _value ) {
			...
			this->print_debug_info("update_database_status() update error, retry update : set : [%s], current : [%d]\n",str_status.c_str(), _value);
		}
	}
