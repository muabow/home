...
/*
	...
	*/	
	...
	bool PcmCapture::setPcmHandle(void) {
		...
		if( (err = snd_pcm_open(&this->tCaptureHandle, this->deviceName, stream, SND_PCM_NONBLOCK)) < 0 ) {
		...
		if( (err = snd_pcm_hw_params_set_format(this->tCaptureHandle, tCaptureParams, format)) < 0 ) {
		...
		// TODO,
		snd_pcm_uframes_t periodsize = this->chunkSize ;
		snd_pcm_uframes_t buffersize;
		snd_pcm_uframes_t exact_buffersize;
		         
		snd_pcm_hw_params_set_period_size_near(this->tCaptureHandle, tCaptureParams, &periodsize, 0);

		buffersize = (periodsize * this->periodSize) / 4;
	...
	void PcmCapture::execute(void) {
		...
		while( true ) {
			...
			if( (tPcmAvail = snd_pcm_avail(this->tCaptureHandle)) == -EPIPE ) {
				snd_pcm_prepare(this->tCaptureHandle);
				fprintf(stderr, "PcmCapture::execute() snd_pcm_prepare() prepared\n");
							
				snd_pcm_wait(this->tCaptureHandle, -1);
