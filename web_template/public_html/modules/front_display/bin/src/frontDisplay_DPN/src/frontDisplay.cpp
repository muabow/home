...
#include "OledTestPage.h"
#include "EMGChannelPage.h"
	...
	try {
		...
		if (networkInterfaces.size() == 1) {
			...
			for(std::vector<std::string>::iterator it = networkInterfaces.begin(); it != networkInterfaces.end(); ++it, i++)
			{
				...
				//display.add(new OledTestPage(networkInterface, PRIORITY[i]));	// Usage
				//display.add(new EMGChannelPage(networkInterface, PRIORITY[i]));	// Usage
			}
		}
