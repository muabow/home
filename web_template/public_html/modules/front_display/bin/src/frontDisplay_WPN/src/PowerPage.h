...
inline bool isFileExistPower (const std::string& name) {
	struct stat buffer;   
...
class PowerLogo {
public:
	PowerLogo(const string path, const string filename) throw (const char*) {
		string logoFile = path + "/" + filename + ".bmp";
		if (isFileExistPower(logoFile) == false) {
			logoFile = path + "/resource/power.bmp";
		}
		printf("[PowerLogo File]\n");
		printf("Filename = %s\n", logoFile.c_str());
		...
		if (memcmp(buf, "BM", 2) != 0) {
			bitmapStream.close();
			throw "Unable to read PowerLogo ile";
		}
...
class PowerLogoPage: public AbstractPage {
private:
	void init() throw (const char*);

public:
	PowerLogoPage(const string path, const string filename ) throw (const char*);
	virtual ~PowerLogoPage() throw (const char*) {};
...
private:
	...
	PowerLogo _powerlogo;

	const string _path;
};

#endif	// __POWER_PAGE_H__
