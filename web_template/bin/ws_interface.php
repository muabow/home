<?php
	...
	class WebsocketInterface {
		...
		const SOCKET_RCV_TIMEO_SEC	= 0;
		const SOCKET_RCV_TIMEO_MSEC	= 100000;
		...
		const WS_ROUTE_CASE_EACH					= 0;
		const WS_ROUTE_CASE_ONLY_WEB					= 1;
		const WS_ROUTE_CASE_ONLY_NATIVE					= 2;
		const WS_ROUTE_CASE_ALL						= 3;
		...
		*/
		...
		function on_event_handler($_clientID, $_socket) {
			...
			socket_set_option($_socket, SOL_SOCKET, SO_RCVTIMEO, array('sec' => self::SOCKET_RCV_TIMEO_SEC, 'usec' => self::SOCKET_RCV_TIMEO_MSEC));
		...
		function _recv_client_ps_data($_clientID, $_socket) {
			...
			if( ($bytes = @socket_recv($_socket, $header_data, $size_common_header, MSG_WAITALL)) === false ) {
				if( $bytes == 0 ) {
					$this->log("disconnected by peer, remove client : [{$_clientID}]/" . $this->get_client_uri($_clientID) . "\n");
					$this->remove_client($_clientID);
		...
		function _send_type_ps_session($_header, $_data) {
			...
			foreach( $this->fds_reads as $id => $client ) {
				...
				if( ($is_route == false) && ($is_route_wscif == false) ) {
					continue;
				}
		...
		function get_ws_route_case_str($_case) {
			switch( $_case ) {
				case self::WS_ROUTE_CASE_EACH 		: return "to each"; break;
				case self::WS_ROUTE_CASE_ONLY_WEB 	: return "to ws ";  break;
				case self::WS_ROUTE_CASE_ONLY_NATIVE 	: return "to ps";   break;
				case self::WS_ROUTE_CASE_ALL 			: return "to all "; break;
		...
		function get_ws_route_case($_src, $_dest, $_case) {
			if( ($_case > self::WS_ROUTE_CASE_ALL) || ($_case < self::WS_ROUTE_CASE_EACH) ) {
				return false;
			}
			
			if( $_case == self::WS_ROUTE_CASE_ALL ) {
				return true; 
			}
			
			$rc = true;
			
			if( ($_src == self::DATA_TYPE_WS_SESSION) || ($_src == self::DATA_TYPE_WS_SOCKET)) {
				if( ($_dest == self::DATA_TYPE_WS_SESSION) || ($_dest == self::DATA_TYPE_WS_SOCKET) ) {
					if( ($_case == self::WS_ROUTE_CASE_EACH) || ($_case == self::WS_ROUTE_CASE_ONLY_NATIVE) ) {
						$rc = false;
					}
					
				} else {
					if( $_case == self::WS_ROUTE_CASE_ONLY_WEB ) {
						$rc = false;
					}
				}
				
			} else if( ($_src == self::DATA_TYPE_PS_SESSION) || ($_src == self::DATA_TYPE_PS_SOCKET) ) {
				if( ($_src == self::DATA_TYPE_PS_SOCKET) && ($_dest == self::DATA_TYPE_PS_SOCKET) ) {
					$rc = false;
				}
				
				if( ($_dest == self::DATA_TYPE_WS_SESSION) || ($_dest == self::DATA_TYPE_WS_SOCKET) ) {
					if( $_case == self::WS_ROUTE_CASE_ONLY_NATIVE ) {
						$rc = false;
					}
					
				} else {
					if( ($_case == self::WS_ROUTE_CASE_EACH) || ($_case == self::WS_ROUTE_CASE_ONLY_WEB) ) {
						$rc = false;
					}
				}
			}
